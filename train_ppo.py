...
if __name__ == "__main__":
    ...
    parser.add_argument('--ae-path', type=str, default="trained_models/batch_size=8_epoch=400_latent_dim=32_06-18-2022_14:46:36_AE.pt")
...
class ViZDoomEnv:
    def __init__(self, height, width, num_channels, seed, game_config, render=True, reward_scale=0.1, frame_skip=4):
        # assign observation space

        self.observation_shape = (num_channels, height, width)
...
def make_env(seed):
    def thunk():
        env = ViZDoomEnv(height, width, num_channels, seed, args.gym_id, render=True, reward_scale=args.scale_reward, frame_skip=args.frame_skip)
...
for update in range(1, num_updates+1):
    ...
    for step in range(0, args.num_steps):
        ...
        with torch.no_grad():
            ...
            cv2.waitKey(1)
            value, action, logproba, _ = agent.get_action(z)
    ...
    with torch.no_grad():
        obs_reduced = ae.sample(next_obs.to(device))
        last_value = agent.get_value(obs_reduced).reshape(1, -1)
        if args.gae:
    ...
    for i_epoch_pi in range(args.update_epochs):
        ...
        for start in range(0, args.batch_size, args.minibatch_size):
            ...
            obs_reduced = ae.sample(b_obs[minibatch_ind])
            _, _, newlogproba, entropy = agent.get_action(obs_reduced, b_actions.long()[minibatch_ind])
            ...
            # Value loss
            new_values = agent.get_value(obs_reduced).view(-1)
            if args.clip_vloss:
